...
var canvasWidth = document.getElementById('image').offsetWidth;
...
function prepareSimpleCanvas() {
	...
	$('#canvasSimple').mousemove(
			function(e) {
				if (paint_simple) {
					addClickSimple(e.pageX - this.offsetLeft, e.pageY
							- this.offsetTop, true);
					redrawSimple();
				}
			});
					...
					function(e) {
						...
						xmlhttp.onreadystatechange = function() {
							if (this.readyState == 4 && this.status == 200) {
								...
								for (var i = 0; i < JSON
										.parse(this.responseText).rows.length; i++) {
									document.getElementById('textOutput').innerHTML += JSON
											.parse(this.responseText).rows[i]
											+ "<br>";
								}
								document.getElementById('textOutput').innerHTML += "<br><b>is a prime with "
										+ JSON.parse(this.responseText).rows.length
										* JSON.parse(this.responseText).rows[0].length
										+ " digits</b>";
								clickX_simple = new Array();
								clickY_simple = new Array();
								clickDrag_simple = new Array();
	...
	canvas_simple
			.addEventListener(
					...
					function(e) {
						// Mouse down location
						var mouseX = (e.changedTouches ? e.changedTouches[0].pageX
								: e.pageX)
								- this.offsetLeft, mouseY = (e.changedTouches ? e.changedTouches[0].pageY
								: e.pageY)
								- this.offsetTop;
...
function loadImage() {
	...
	function imageLoaded() {
		...
		// Sending 15times larger image than backend requires. Let backend do
		// resizing.
		canvas.width = 15 * Math.sqrt(1024) * Math.sqrt(img.width / img.height);
		canvas.height = 15 * Math.sqrt(1024)
				* Math.sqrt(img.height / img.width);
